...
if model.status == GRB.OPTIMAL and model.SolCount > 0:
    ...
    AssignImpliesUsed = {(r, k): model.addConstr(Vs[r, k] <= Z[k]) for r in R for k in K}
    ...
    # -----------------------------
    # warm-start: seed Start values
    # -----------------------------
    for k, var in X.items():  
        var.Start = x_start[k]
    for k, var in S.items():  
        var.Start = s_start[k]
    for k, var in C.items():  
        var.Start = c_start[k]
    for k, var in Vs.items(): 
        var.Start = vs_start[k]
    for k, var in Rq.items(): 
        var.Start = rq_start[k]
